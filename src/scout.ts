...
const SCOUT_QUERIES_SYSTEM = `You generate 3-5 broad survey queries for an initial literature scouting pass. These must be generic enough to return a CROSS-SECTION of relevant papers — not narrow questions with embedded assumptions.
...
export async function scout(topic: string): Promise<ScoutDigest | null> {
  ...
  try {
    const { object } = await generateJson({
      ...
      prompt: `Topic: ${topic}\n\nGenerate 3-5 broad survey queries.`,
      maxRetries: 1,
      endpoint: config.endpoints.planner,
    });
    queries = object.queries.slice(0, 5);
  } catch (err: any) {
  ...
  for (const query of queries) {
    ...
    const primary = paged.filter(
      (r: any) => r.provider === "arxiv" || r.provider === "openalex" || r.provider === "semantic_scholar"
    );
    const tiered = sortByTier(primary, (r: any) => r.url);
  ...
  try {
    ...
    const { object } = await generateJson({
      ...
      prompt: `Topic: ${topic}\n\nSources from broad survey queries:\n\n${block}\n\nReturn the structured digest as JSON.`,
      maxRetries: 1,
      endpoint: config.endpoints.planner,
    });
    console.log(
