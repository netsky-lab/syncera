...
export function viewerUidFromRequest(request: Request): string | null {
  ...
  const sessionUid = verifySessionUser(m?.[1])?.uid;
...
export function requireAuth(request: Request): { ok: true } | { ok: false; response: Response } {
  ...
  if (cookie.includes("rl_session=")) {
    const { COOKIE_NAME, verifySessionUser } = require("@/lib/sessions");
    const match = cookie.match(new RegExp(`(?:^|;\\s*)${COOKIE_NAME}=([^;]+)`));
    if (verifySessionUser(match?.[1])) return { ok: true };
  }
  ...
  // configured, browser/API auth should use sessions or API keys.
  const authHeader = request.headers.get("authorization");
  if (!process.env.SESSION_SECRET && basicPass && authHeader?.startsWith("Basic ")) {
...
export function requireBasicAuth(request: Request): { ok: true } | { ok: false; response: Response } {
  ...
  // user accounts exist.
  const basicPass = process.env.BASIC_AUTH_PASS;
  if (!basicPass && !process.env.SESSION_SECRET && !hasStoredUsers()) return { ok: true };
  if (!process.env.SESSION_SECRET && basicPass) {
    const authHeader = request.headers.get("authorization");
...
function hasStoredUsers(): boolean {
  try {
